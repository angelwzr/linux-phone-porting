...
- **2026-09-20**
  - **Mi A3 session harvest.** Five measured rules join from a later port stage: one-direction-dead USB networking classified by device-side counters; a host-side USB id proving neither the running OS nor a failed boot; a hard reset during suspend entry masquerading as a dead gadget, bisected with `pm_test` stages; detached `systemd-run` recovery units and their minimal-PATH false-negative safety nets; module load order as an experiment variable.
  - **Retail-selected HAL variants.** The AOSP source family now ports the backend the retail config actually selects, not the conventionally suggested one.
  - **Host-tool provisioning.** Phase 0 provisions missing host tools through the host OS's own package route; container/WSL setups prove USB passthrough first.
  - **NixOS folded into the general-distro family.** The standalone NixOS source-family bullet merges into "General-purpose and desktop distributions" — the last founding-distro bias in the family list. Nix state-model lessons stay as scoped examples.
  - **Upstream send hygiene.** `/port-upstream` gains send-path verification (self-test send, duplicate-copy disclaimers naming the operative Message-ID, archive fallbacks when Patchwork/lore are unreachable) and first-time-contributor limits as operator-owned ledger actions.
  - **Publication hygiene.** Everything leaving the project — upstream payloads, public docs, issue replies — is swept for operator-supplied secrets (SSIDs, passwords, tokens) and device-unique identifiers (IMEI, serials, MACs), then verified by scanning the outgoing bytes.
  - **Session-swap defaults and remote eyes.** A compositor swap re-opens layer defaults (an unclaimed power key turns a short press into a clean shutdown that reads as a crash); compositor screenshots over the demonstrated channel become an evidence channel; a silent dim device gets a charge check before a boot-failure verdict.
  - **Android-app compatibility.** Planned as a target capability: candidate layers gated on kernel requirements (binder, PSI, GPU stack), raced on-device, winner benchmarked against the native session — one port measured the container faster than its own phone session.
