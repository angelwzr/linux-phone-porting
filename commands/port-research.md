...
- If the target's users must run Android apps, plan the compatibility layer explicitly: enumerate candidate implementations (waydroid, lepton-class Android-runtime ports, peers), gate each on its mainline kernel requirements — binder, PSI/memfd, GPU stack — against the kernel being built, and keep an on-device race plus a native-session benchmark in the plan. Adoption is a measurement, not a default.
