...
This runs the `linux-phone-porting` phases in a loop rather than one at a time. It does not replace them — each iteration calls them, and their rules still bind. Do not restate the source families here; `/source-sweep` owns them. Do not edit-and-hope either: every change lands by reading the exact current lines and writing the exact replacement — an edit that has to be "repaired" afterwards is redone from a fresh read, not patched over.
...
3. **Apply.** Prefer the cheap path every time: tar-pipe files, unbind/rebind, `insmod`, rebuild only the DTB. Flash only when nothing cheaper can test the hypothesis. If the change needs a reboot and the device is reachable (ssh, USB, fastboot), reboot it yourself and continue — operator hands are only for what no channel reaches.
4. **Verify.** `/evidence-sweep` on the result. Read it against the refutation criterion written in step 2, not for confirmation. If a step needed the operator's hands (moving a card, replugging, a button hold), name the exact action and direction when asking, then verify it from an observable signal before continuing — and when the confirming signal is a disappearance (the device powers off, a channel drops), "no response" is ambiguous between not-done and done: watch the drop as an event, or get the operator's done-confirmation plus a failing probe, never silence alone. Poll on a short interval and re-ask within a bounded wait — never freeze on a request the operator may not have registered as an action item.
