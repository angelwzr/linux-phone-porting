...
**Before the first iteration.** Confirm the phase 0 artefacts exist — stock DTB, vendor configs, firmware layout, verified backup with hashes. If they do not, run `/port-setup` and stop; there is nothing to research against, and the loop would research some other board that shares this SoC. State the target as one falsifiable sentence: what the device will do that it does not do today, and how you will observe it. Then ask the operator how many refutations the loop is budgeted: **5, 15, or unlimited** — record the answer in the ledger header; every later stop decision reads against it.
