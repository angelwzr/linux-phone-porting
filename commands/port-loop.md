...
**Authorization and resumption.** Apply the skill's scoped permission, physical-test envelope and protection rules before every device action. Unlimited refutations do not authorize unbounded physical output or override a pause. Record an explicit deferral as paused, with the exact prerequisite, permitted remaining work and final safe state. Resume the same invocation only after reconciling its records and satisfying the prerequisite with required operator authorization; retain its recorded budget. A new invocation still asks for a fresh budget.
...
5. **Record.** One line per iteration: hypothesis, source that backed it, what was applied, outcome, and on a refutation what it rules out. Keep the current target, tested revision, acceptance results, unresolved faults, authorization state and next action consistent under the skill's completion-record rule; preserve superseded checkpoints separately. This ledger is the loop's memory — a later iteration that re-proposes a refuted change is a bug.
...
- The operator defers or withdraws permission, a required physical prerequisite is unmet, or safe limits/protection cannot be established. Pause the affected work and report what is missing; perform only explicitly permitted remaining work. A working control channel is not permission to reboot or actuate.
- **The refutation budget is exhausted.** The budget comes from the operator at invocation (5, 15, or unlimited) and lives in the ledger header. Widening is not a stop: every third refutation on the same symptom forces a scope change — re-enter `/port-research` on the layer above, or on the primary artefacts directly where no precedent exists, never the same layer again — and the loop continues while budget and authorization remain. On a finite budget, stop when it is spent and hand back what the refutations collectively rule out; on unlimited, keep looping subject to every stop/pause condition listed here.
- The next step would write to the bootloader, modem NV/EFS, persist, the partition table, or anything else whose loss is not recoverable from the phase 0 backup, including excluded userdata. Boot, dtbo and modules may be written only within the authorized experiment and its gates. Say what you want to write and why, and wait for the required separate authorization.
...
Report at the end: the ledger, consistent current device and authorization state, acceptance evidence, remaining limitations/new faults, and either what made it work, what the refutations collectively rule out, or the exact pause/resume prerequisite. A fulfilled target does not erase an unexplained regression or authorize another experiment. **Name any shareable upstream candidates** (portable fixes, bindings, drivers) for the user — surface them only; `/port-upstream` prepares them, and nothing is submitted automatically.
