...
| ----------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Before the first flash**    | Flash the first build straight away. If it corrupts modem NV/EFS or persist, the phone never registers on a network again — and those partitions are in no image on the internet.                                      | Phase 0 first: pin the exact variant, back up every partition except userdata, hash and restorability-check the backup, health-check the storage, harvest the stock DTB, firmware layout and vendor configs. The riskiest step happens after the safety net exists. |
| **Something fails**           | Read the log you happen to have, form a theory, change something, rebuild, reflash. If the debug knobs were off, the evidence for the next attempt is gone — and each attempt costs a build + flash + boot (5–10 min). | Debug knobs on _before_ reproducing; full dmesg captured; pstore taken from the path systemd actually leaves it in; what is flashed confirmed by read-back hash, not notes.                                                                                         |
| **Finding the fix**           | Take the first plausible search hit for the symptom — often a different variant of the same phone, with a different panel, touch controller or modem.                                                                  | Research sweeps every source family in order — vendor mainline project, GPL OEM kernel, AOSP, postmarketOS, Halium, Mobian, NixOS — and diffs sister devices' board files against the phase 0 inventory. Adopt only what the diff confirms.                         |
| **All sources come up empty** | "Nothing exists, this port is impossible." Work stops or gets handed back.                                                                                                                                             | An empty sweep means nobody has published the work — the design gets derived from the primary artefacts (vendor driver, stock DTB, closest mainline driver) and the missing piece gets written. That is the normal terminal state of bring-up.                      |
| **Applying the fix**          | Change, rebuild (tens of minutes), flash, find the build broke on a renamed symbol, repeat. Several variables move at once, so a pass or fail proves nothing.                                                          | One variable per flash, falsifier built before the fix, cheap runtime tests over reflashes, and the pre-build battery — patches apply, symbols exist, config options exist — before any full rebuild. Three refutations route back to research, not more guessing.  |
...
- **2026-09-14**
  ...
  - **Context and docs:** `SKILL.md` and the update command compressed with all rules, measurements and source families verified intact by before/after subagent tests; README adds a "Without the skill vs with it" workflow comparison.
