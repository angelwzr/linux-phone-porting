...
The target OS is selected by its release, kernel requirements and integration contract, not a distro compatibility list. Named projects below are research destinations. The method covers mainline Linux phone bring-up and OS userspace integration, including non-GNU userspace; it does not certify a platform/device combination or cover non-Linux kernels.
...
| **Finding the fix**           | Take the first plausible search hit for the symptom — often a different variant of the same phone, with a different panel, touch controller or modem.                                                                  | Research the selected target platform alongside vendor mainline, GPL OEM kernel, AOSP, postmarketOS, Halium, Mobian, NixOS and general-purpose/desktop distribution sources; map manufacturer datasheets, schematics, FCC exhibits and Scribd documents to the inventory. Compare exact changes and sister-device components; ARM availability alone is not compatibility.    |
...
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **0. Set up**    | Unlocked bootloader required. Pin the exact variant, target OS integration contract, interface/session and boot topology. Back up every partition but userdata; exclusion does not authorize wiping it. Verify storage health, inventory components and harvest stock evidence. Discover and prove supported control/recovery operations, not assumed tools.                                                 |
...
- **2026-09-15**
  - **Hardware research:** persistent component–wiring–document inventory; explicit manufacturer, schematic, FCC, repair-archive and Scribd discovery; document types, actual access and unresolved identities kept distinct.
  - **Evidence correction:** withdraws the invalid static-camera-register conclusion. Requires wiring reconciliation before timing sweeps and qualifies stock-DT authority; configuration is not measured physical truth.
  - **Experiment discipline:** separate timing/data clocks, controlled media ownership, linear RAW photometry and minimal helper-free boot integration. Setup/research commands retain one authoritative source list. Measurements and verification details are in the [full changelog](CHANGELOG.md).
  - **Portability and safety:** target OS contracts replace distro assumptions; transports, crash collection and deployment follow the actual platform. Physical-test limits, scoped permission, calibration privacy and consistent completion records are mandatory. Userdata exclusion is not permission to wipe.
  - **Upstream workflow (preview):** new `/port-upstream` command — existing-work check, destination rules, verified submission bundle, user-owned maintainer interaction, prepared → released ledger. Preview: the underlying workflow is proven by real GitHub issue/PR and mailing-list submissions, but the command itself has not yet driven one end-to-end; expect iteration.
  - **Compression:** `SKILL.md` cut 7,442 → 5,884 words (−21%) and `/port-setup` trimmed, with every rule, measurement and source family preserved and verified by a 45/45 scenario battery.
