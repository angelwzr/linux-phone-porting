...
- **`/port-update`** — _a newer kernel, app or component release is out and the port should take them in one pass._ Inventories pinned versions as rollback anchors, frames targets with a pre-bump regression sweep, orders components by dependency, and applies one class per gated deployment cycle — patch absorption checks, config migration, per-component verification and a regression pass over already-working capabilities included. Stops and holds on the first failed verification; `/port-cleanup` closes the batch.
