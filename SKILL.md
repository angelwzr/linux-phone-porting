...
3. **Web research across all six source families** (parallel subagents work well here; if a web-research skill is in that list, prefer it for the caching, since sessions re-read the same pages):
   - **Mainline** — lore.kernel.org, the driver source, DT bindings, and the dtsi of the closest sibling SoC (same family, or same RPM generation). Before scoping any "port the downstream driver" campaign, sweep what mainline already has for the interface: its exported API, whether it reaches userspace or is kernel-internal only, and its gates — machine allowlists, "pre-loaded by the bootloader" semantics, built-in rather than module (measured: one such sweep cut a 9.4k-line downstream port to a ~300-line probe module, because mainline already carried the interface kernel-internally behind an allowlist — the plan became drive the existing API, not port it).
