...
- **2026-09-21**
  - **`/port-update` + `/port-cleanup` close-out hardening.** Kernel-version moves deploy kernel + matching module tree in one gated cycle; empty checks are recorded as empty in ledgers and the health gate; the update close-out names the agent context files; cleanups pin protected closures as GC roots before store reclamation, classify orphaned build-output links, verify no dangling links remain, commit their own tree changes, and persist the report as a file.
  - **Self-executing, ledger-gated source sweep.** Phase 2 now triggers itself: precondition met ⇒ open a sweep ledger (one row per question × source family) and start — no operator prompting, no named tools. A question closes only when every family row carries a finding or a recorded gap; per-family parallel fan-out is the default.
  - **Self-driving phase chain.** Each phase advances to the next itself, and every phase command ends in an explicit handoff (`/port-setup` asks what to pursue first, `/evidence-sweep` starts the sweep, `/source-sweep` and `/port-research` carry the top hypothesis into `/flash-gate`, `/flash-gate` routes the outcome). Stops are only the named operator decisions: identity, interface/session, boot paths the operator completes, budget, destructive writes, physical hands, pauses. Phase 0's questions are unconditional — existing repos, layers, staged OS images or prior recorded "decisions" are candidates to offer, never answers.
  - **Raw-storage access ≠ unlocked-like.** An EDL-class firehose on a locked device is a write/recovery route, not a boot path; the class gate is met only by a non-stock image demonstrated to boot on the device, after which the posture is retail-unlock-equivalent with the route as named recovery.
  - **Task-list refresh on entry.** Every phase entry and command invocation replaces the session's task list with its own steps before starting work — first item in progress, stale items superseded.
  - **Exploit-booted session mechanics.** Three-way pwn-session liveness gate, session-lifetime staging, per-build payload-PID success signals, raw descriptor reads over truncated host-tool renderings, chain-binary provenance, entitlement-gated refusal ≠ absence.
  - **Firmware-boot phase 0.** Encryption state as a wipe/resize gate; the stock OS's ssh as the harvest/staging control channel; driver-store firmware harvest pre-wipe; ACPI harvest with absences read as architecture; stock-OS parent-chain walk as the port→controller oracle; DMI untrusted for SoC generation — CPU identity discriminates.
  - **Firmware-boot bring-up gates.** DTB-injection delivery checks (lockdown, auto-DTB databases, dtbloader lists, installed-boot persistence), no hand-authored memory nodes, sibling-DTB control boots, BootNext reset semantics, declared verdict surfaces for serial-less devices, live-media falsifier ladder with the menu render as a free display test.
  - **Evidence additions.** Validate the readback method itself (`iflag=direct` misreported on one host's USB storage); cross-arch crash attribution; compositing-tax decomposition; multi-session hunt infrastructure; harnesses treat missing evidence as FAIL; reserved-memory map diffs; crash-log kernel fingerprint as the no-OEM-drop oracle; CPU-arch gating for prebuilt images; suspend pre-flight; remote-processor DMA staging; prose holds no volatile numbers.
  - **`/port-upstream` resubmission mechanics.** Fresh-repo regeneration against the posted mboxes, In-Reply-To threading, tag survival on content-identical patches only.
