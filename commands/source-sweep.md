---
description: Research a symptom or a new capability across all seven source families before writing any code (phase 2 of linux-phone-porting)
...
- `wigolo`, for the source sweep below — its cache matters, since these pages get re-read across sessions; fallback: plain web search.

Then sweep **all seven** source families, in parallel subagents where possible. The point is that the fix comes from all of them, not from the first plausible hit:
...
When all seven come back empty, derive the candidate hypotheses from the primary artefacts — the vendor driver source, the stock DTB, the closest mainline sibling driver — and rank them like any other.
